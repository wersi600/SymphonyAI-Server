...
def _exact_lyrics_contract(payload: dict) -> tuple[str, int, str]:
    """Preserve lyric layout and reject any transport-time mutation."""
    lyrics = str(payload.get("lyrics") or "").replace("\r\n", "\n").replace("\r", "\n")
    line_count = 0 if not lyrics else len(lyrics.split("\n"))
    digest = hashlib.sha256(lyrics.encode("utf-8")).hexdigest()
    expected_lines = int(payload.get("lyrics_line_count") or line_count)
    expected_digest = str(payload.get("lyrics_sha256") or digest).lower()
    if expected_lines != line_count or expected_digest != digest:
        raise ValueError(
            "가사 원문 무손실 검사 실패: "
...
def _submit_worker(job: dict) -> dict:
    response = requests.post(
        ...
        json={
            ...
            "title": job["title"],
            "lyrics": job["lyrics"],
            "lyrics_line_count": job["lyrics_line_count"],
            "lyrics_sha256": job["lyrics_sha256"],
            "prompt": job["prompt"],
...
def create_song(background_tasks: BackgroundTasks, payload: dict = Body(...)):
    ...
    try:
        lyrics, lyrics_line_count, lyrics_sha256 = _exact_lyrics_contract(payload)
